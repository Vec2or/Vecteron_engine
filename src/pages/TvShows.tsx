...
const TvShows = () => {
  const { tvShows, loading } = useMoviesDB();
  const [selectedGenre, setSelectedGenre] = useState<string>('');
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');

  const filteredShows = selectedGenre 
    ? tvShows.filter(show => show.genre.toLowerCase().includes(selectedGenre.toLowerCase()))
    : tvShows;
      ...
      <div className="min-h-screen bg-background relative overflow-hidden">
        ...
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="mt-4 text-muted-foreground">Loading TV shows...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 space-y-4 lg:space-y-0">
            <div>
              ...
              <p className="text-muted-foreground">Binge-watch the best series from our collection</p>
            </div>
          ...
          {filteredShows.length === 0 ? (
            ...
            <div className={
              ...
              {filteredShows.map((show) => (
                <div key={show.id} className={viewMode === 'list' ? 'w-full' : ''}>
                  <MovieCard {...show} />
                </div>
              ))}
            </div>
